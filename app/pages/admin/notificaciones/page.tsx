...
interface Notificacion {
  ...
  tipo_operacion: 'eliminacion' | 'transferencia';
  descripcion: string;
      ...
      <main className="flex-1 overflow-y-auto p-4 bg-gray-50">
        ...
        <div className="max-w-4xl mx-auto">
          {notifications.length === 0 ? (
            <div className="text-center py-8"><p className="text-gray-500 text-lg">No hay notificaciones pendientes</p></div>
          ) : (
                ...
                <div key={noti.id_notificacion} className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2 ml-4">
                      <button onClick={() => { setNotification(noti); setShowConfirm(true); }} className={`p-2 rounded-lg ${noti.tipo_operacion === 'eliminacion' ? 'text-red-600 hover:bg-red-50' : 'text-blue-600 hover:bg-blue-50'}`}>
                        {noti.tipo_operacion === 'eliminacion' ? <TrashIcon className="h-5 w-5" /> : <TruckIcon className="h-5 w-5" />}
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {showConfirm && notification && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            <h3 className="text-xl font-bold mb-4">Confirmar {notification.tipo_operacion === 'eliminacion' ? 'Eliminación' : 'Transferencia'}</h3>
            <p className="mb-4">¿Estás seguro de querer {notification.tipo_operacion === 'eliminacion' ? 'eliminar' : 'transferir'} este bien?</p>
            <div className="flex gap-4 justify-end">
              <button onClick={() => { setShowConfirm(false); setNotification(null); }} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded" disabled={isProcessing}>Cancelar</button>
              <button onClick={handleAction} className={`px-4 py-2 text-white rounded ${notification.tipo_operacion === 'eliminacion' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`} disabled={isProcessing}>{isProcessing ? 'Procesando...' : 'Confirmar'}</button>
            </div>
          </div>
        </div>
      )}
